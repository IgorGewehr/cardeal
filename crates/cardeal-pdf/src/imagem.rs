...
pub fn decodificar_logo(bytes: &[u8]) -> Result<Imagem, ErroPdf> {
    let imagem = image::load_from_memory(bytes)
        .map_err(|e| ErroPdf::Logo(e.to_string()))?;
    ...
    let imagem = if imagem.width() > LARGURA_MAX {
        ...
        imagem.resize(LARGURA_MAX, altura.max(1), image::imageops::FilterType::Lanczos3)
    } else {
        imagem
    };
    ...
    for px in rgba.pixels() {
        ...
        let mistura = |canal: u8| -> u8 {
            ...
            ((c * alfa + (1.0 - alfa)) * 255.0).round().clamp(0.0, 255.0) as u8
        };
        rgb.push(mistura(r));
        rgb.push(mistura(g));
        rgb.push(mistura(b));
