...
**Total: 409 testes passando via `cargo test --workspace` (unit + integração SQLite + doctests). Tudo verde.**
...
Os outros 14 crates do workspace (`cardeal-protocol`, `cardeal-server`, `cardeal-cliente`,
`cardeal-ui`, `cardeal-analytics`, `cardeal-testkit`, `cardeal-desktop`,
...
mesmo com a maior parte do sistema ainda não escrita — ver §4.
